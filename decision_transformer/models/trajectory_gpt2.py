...
def find_pruneable_heads_and_indices(heads, n_heads, head_size, already_pruned_heads):
    ...
    Args:
        heads (:obj:`List[int]`): List of heads to prune.
        n_heads (:obj:`int`): The number of heads in the model.
        head_size (:obj:`int`): The size of each head.
        already_pruned_heads (:obj:`Set[int]`): A set of already pruned heads.
    Returns:
        :obj:`Tuple[Set[int], torch.LongTensor]`: A tuple with the remaining heads and the indices of the heads to be
        pruned.
    """
...
def prune_conv1d_layer(layer: nn.Module, index: torch.LongTensor, dim: int = 1):
    """
    Prune a Conv1D layer to keep only entries in index.
    Used to remove heads.
    Args:
        layer (:obj:`torch.nn.Module`): The layer to prune.
        index (:obj:`torch.LongTensor`): The indices to keep in the layer.
        dim (:obj:`int`, `optional`, defaults to 1): The dimension on which to keep the indices.
    Returns:
        :obj:`torch.nn.Module`: The pruned layer as a new layer with :class:`~torch.nn.Parameter`.
    """
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.use_layers = config.use_layers if hasattr(config, 'use_layers') else None
